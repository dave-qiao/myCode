...
// 分页函数
  ...
  render() {
    ...
    let _shop_address_detail = '';//TODO: 废弃
    let _shop_address = '';//TODO: 废弃
    let _shop_mobile = '';//TODO: 废弃
    let _state = '';//商户状态
    let _name ='';//联系人
    let _verify_state = '';//_verify_state
    let _Source_of_registration = 'ios';//TODO: 废弃
    let _mobile ='';
    ...
    if(d_seller.created_at){
      let date = window.tempAppTool.utcToDate(d_seller.created_at).date;
      let time = window.tempAppTool.utcToDate(d_seller.created_at).time;
      time.length = 2;
    ...
    let n = citys.index.indexOf(d_seller.city_code);
    if(n!=-1){
...
let total1 = data.length;
    ...
    return (
      <Form horizontal  className="main-form">
        ...
        <Table columns={columns} dataSource={data}  loading={false} pagination={pagination} />
  ...
  return (
    <div className="con-body">
      <div className="bd-header">
        <MainForm details={business_seller.list_details}
                  dispatch = {dispatch}
                  shops_info ={business_seller.shops_info} />
      </div>
    </div>
  );
};
