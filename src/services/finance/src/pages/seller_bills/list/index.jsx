...
// 日账单查询
  dayOnSearch = (values) => {
    ...
    // 触发action
    dispatch({type: SELLER_BILLS.find, payload: {...this.querys,page}});
  }
// 月账单查询
  monthOnSearch = (values) => {
    ...
    Object.assign(this.querys,values);
    // 触发action
    dispatch({type: SELLER_BILLS_MONTH.find, payload: {...this.querys,page}});
  }
// 日账单的分页函数
  dayOnPageChange = (page) => {
    ...
    this.page = page;
    dispatch({type: SELLER_BILLS.find, payload: {...querys,page}});
  }
// 月账单的分页函数
  monthOnPageChange = (page) => {
    ...
    this.page = page;
    dispatch({type: SELLER_BILLS_MONTH.find, payload: {...querys,page}});
  }
 ...
 callback = (key) => {
  ...
  render() {
    // 从props里面获取信息
    const {business_seller_bills, dispatch, business_public} = this.props;
    // 从model获取details信息
    const { Day_list_tables,Month_list_tables} = business_seller_bills;
...
function mapStateToProps({business_seller_bills,business_public}) {
  return {business_seller_bills, business_public};
};
