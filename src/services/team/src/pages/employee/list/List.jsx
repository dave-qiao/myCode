  ...
  const columns = [
    {
      ...
      "key": "code",
      render: (text, record) => (
        <Link style={{ color: 'rgb(88,226,194)' }} to={"/team/employee/list/detail?id=" + record.id}>{text}</Link>
      )
    }, {
      "title": "姓名",
      "dataIndex": "name",
    ...
    },
    {
      ...
      render: (text, record) => {
        if (text) {
          let date = window.tempAppTool.utcToDate(text).date;
          let time = window.tempAppTool.utcToDate(text).time;
          time.length = 2;
    ...
    },
    {
      ...
      render: (text, record) => {
        return (
          <p>
            <Link style={{ color: 'rgb(88,226,194)' }} to={"/team/employee/list/edit?id=" + record.id}>编辑</Link>
          </p>
        )
      }
    }
