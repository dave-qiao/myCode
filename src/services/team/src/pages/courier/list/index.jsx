...
class View extends Component {
  ...
  onSearch = (values) => {
    ...
    const { city } = this.props.business_courier;
    const city_code = city;
    const { dispatch, page } = this;
    Object.assign(this.querys, values);
    dispatch({ type: COURIER.find, payload: { ...this.querys, page, city_code } });
  };
  ...
  onSelect = (info)=> {
    ...
    if (info == '所有骑士') {
      ...
      message.error('请选择左侧团队或者骑士中的一项', 2);
    } else {
      let listType = info[0];
      if (info.length != 0) {
        this.setState({
        ...
        if (this.state.teamSearchValues !== '') {
          ...
          dispatch({ type: 'business_courier/querys', payload: { ...this.querys, team_id, city_code } });
        } else {
          ...
          <div className={style.courierAll}>
            <Col sm={5}>
              ...
              <div>
                ...
                <Tree className={style.courierBlockTree} onSelect={this.onSelect} selectedKeys={defaultSelect}
...
mapStateToProps({ business_courier }) {
