...
import {CORUIER} from '../../../ActionsName.js';
...
class View extends Component {
  ...
  render() {
    const {statictics_shipments_courier,business_public} = this.props;
    const {list_tables} = statictics_shipments_courier;
    const {page, onSearch, onPageChange} = this;
    console.log('234',business_public)
    console.log('couriers', business_public.couriers)
    const searchProps = {
      onSearch,
      couriers: business_public.couriers
    };
    const tableProps = {
      ...list_tables,
      onPageChange,
...
function mapStateToProps({statictics_shipments_courier, business_public}) {
  return {statictics_shipments_courier, business_public};
};
