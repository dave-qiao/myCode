...
//根据商家类型获取商家列表
import { getSellerByType } from '../../../aoao-core-api-service/src/retail';
...
module.exports = {
	...
	effects: {
		...
		*['shipments_detail/AreasTotalList'](params) {
		...
		//根据商家类型获取商家列表
		*['shipments_detail/getSellersByType'](params) {
			...
			yield put({
				type: 'getSellersByType/success',//对应reducer的函数名
				payload: resultData
			});
		},
		//清空商家列表
		*['shipments_detail/clearSellers'](params) {
			yield put({
				type: 'clearSellers/success',//对应reducer的函数名
				payload: {}
			});
		},
	},
	reducers: {
		...
		//根据类型获取商家
		['getSellersByType/success'](state, action) {
			const { sellers } = state;
			return {
				...state,
		...
		['clearSellers/success'](state, action) {
			const { sellers } = state;
			return {
				...state,
				sellers: []
