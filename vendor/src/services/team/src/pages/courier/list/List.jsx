...
const List = ({ data, _meta, page, loading, onPageChange, listType, dispatch, tabs, city }) => {
  ...
  function teamListDelete(index) {
    ...
    dispatch({ type: 'removeTeamMember', payload: { courier_ids, tabs, city_code } });
  };
        ...
        render: (text, record) => (
          <Link style={{
            color: 'rgb(88,226,194)'
                ...
                onClick={setTreeSelect.bind(this, record.id)}>{text}</Link>
        )
      }, {
        "title": "姓名",
        "dataIndex": "name",
            ...
            <p>
              {
                text == 1 ? <Link style={{
                  color: 'rgb(88,226,194)'
                                  ...
                                  onClick={setTreeSelect.bind(this, record.id)}>审核</Link> :
                  <Link style={{
                    color: 'rgb(88,226,194)'
                  }} to={"/team/courier/list/edit?id=" + record.id}
                        onClick={setTreeSelect.bind(this, record.id)}>编辑</Link>
    ...
    columns = [
      ...
      },
      {
        ...
        render: (text, record, index) => {
          return (
            <p>
            <span onClick={teamListDelete.bind(this, index)}
                  style={{ color: 'rgb(88,226,194)', cursor: 'pointer' }}>删除</span>
            </p>
          )
        }
      }
