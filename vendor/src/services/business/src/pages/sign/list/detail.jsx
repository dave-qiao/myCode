...
// 显示定价模式的函数
  function show_price_mode() {
    const _result = [
      <FormItem key="price_mode1" label="定价模式" {...Lay_6_16}> {stateNames.price_mode[list_details.price_mode]} </FormItem>,
    ];
    if (list_details.price_mode === 2) {
      _result.push(
        <FormItem key="plan_type1" label="定价方案" {...Lay_6_16}> {stateNames.plan_type[list_details.plan_type]} </FormItem>,
      )
    }
    return _result;
  }
