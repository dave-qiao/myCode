...
class MapEditComponent extends React.Component {
  ...
  componentDidMount() {
    ...
    //初始化地图
    const mapObject = new BMap.Map('map');
  ...
  loadMapComponent = (areaDraftDetail) => {
    ...
    //判断是否有父级区域(如果有则绘制)
    const hasParentArea = (areaDraftDetail.parent_area) ? true : false;
    //绘制父级区域
    if (hasParentArea === true) {
      //判断数据是否存在 TODO: js语言，连个级联属性判断空都没有。稍后需要封装工具函数。
      if (areaDraftDetail.parent_area.region_info.bd_polygon) {
        //获取父区域坐标
    ...
    //判断是否有坐标数据
    const hasCoordinates = (areaDraftDetail.region_info) ? true : false;
    //绘制区域
    if (hasCoordinates) {
      //获取当前区域坐标
      ...
      <Row>
        <div className="form-divider-header" style={{ margin: '-10px 10px 8px 0px', width: '97%', paddingLeft: '24px' }}>
          ...
          <Col sm={12} style={{ textAlign: 'right' }}>
            ...
            {/* { submitSuccessMessage ? <span style={{ color: '#58e2c2' }}><Icon type="exclamation-circle-o" /> {submitSuccessMessage}</span> : ''} */}
  ...
  renderMapDetailComponent = () => {
    ...
    return (
      <Row>
        <Col sm={6} offset={-1} style={{ textAlign: 'left' }}>
          <FormItem label="区域名称 " {...formItemLayout}>
            <Input type="hidden" {...getFieldProps('areaId', { initialValue: areaDraftDetail.id })} />
            <Input type="hidden" {...getFieldProps('vendorId', { initialValue: vendorId })} />
            <Input type="hidden" {...getFieldProps('cityCode', { initialValue: cityCode })} />
            <Input type="hidden" {...getFieldProps('parentAreaId', { initialValue: parentAreaId })} />
            <Input placeholder="请输入区域名称" size="small" {...getFieldProps('areaName', { initialValue: areaDraftDetail.name, rules: [{ required: true, message: '请填写区域' }, { validator: checkAreaName }] })} />
          </FormItem>
        </Col>
        <Col sm={6} style={{ textAlign: 'left' }}>
          <FormItem label="状态" {...formItemLayout}>
            <RadioGroup {...getFieldProps('areaState', { initialValue: defaultAreaState })} >
  ...
  render() {
    ...
    return (
      <Col sm={24} className={`${style.height100}`} style={{ padding: '10px' }}>
        <Form horizontal className={`${style.height100} main-form`}>
          {renderMapTitleComponent()}
          {renderMapDetailComponent()}
          {renderMapComponent()}
        </Form>
