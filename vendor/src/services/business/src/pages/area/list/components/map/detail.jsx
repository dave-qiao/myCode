...
class MapDetailComponent extends React.Component {
  ...
  componentDidMount() {
    ...
    //初始化地图
    this.private.mapObject = new BMap.Map('map');
  ...
  loadMapComponent = () => {
    ...
    if (region_info.bd_polygon.coordinates.length) {
      if (region_info.bd_polygon.coordinates[0].length) {
        ...
        //判断是否有父级区域(如果有则绘制)
    const hasParentArea = (areaDetail.parent_area) ? true : false;
        //绘制父级区域
    if (hasParentArea === true) {
          //判断数据是否存在 TODO: js语言，连个级联属性判断空都没有。稍后需要封装工具函数。
      if (areaDetail.parent_area.region_info.bd_polygon) {
            //获取父区域坐标
    ...
    return (
      <Row>
        <Col sm={8} style={{ textAlign: 'left' }}>
          <FormItem label="区域ID" {...formItemLayout}>{areaDetail.id}</FormItem>
        </Col>
        <Col sm={6} style={{ textAlign: 'left' }}>
          <FormItem label="状态" {...formItemLayout}>{AreaState.description(areaDetail.state)}</FormItem>
        </Col>
        <Col sm={10} style={{ textAlign: 'right' }}>
          {
            (isShowEdit === true) ? <Button type="primary" onClick={(e) => { onClickAreaEdit(areaDetail.id) }}>编辑</Button> : ''
          }
        </Col>
      </Row>
    )
