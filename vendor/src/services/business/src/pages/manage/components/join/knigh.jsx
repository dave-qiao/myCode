...
class OrderSingle extends Component {
  ...
  showModal = ()=> {
    ...
    const relate_type = 20; //直营类型 枚举值 10直营 20加盟
  ...
  onPageChange = (current) => {
    ...
    /* dispatch({
     type: 'getKnightRuleDetailE',
     payload: { area_id, vendor_id, state, page, limit },
     });*/
  ...
  render() {
    ...
    if (upData == true) {
      ...
      const seller_id = sessionStorage.getItem('sellerId');
      const limit = 1000;
      const vendor_ids = sessionStorage.getItem('vendorId');
      // 通知model 层 通过区域id 获取此区域的分单规则详情信息
      dispatch({
        type: 'getKnightRuleDetailE',
        payload: { area_id, vendor_id, state, contract_id, seller_id },
      });
    ...
    return (
      <div className="con-body main-list">
        <div className={style.reset}>
          <Link to={`/business/manage/affiliates/info?id=${seller_id}`}>
            <span className={`${style.tabs}`}>商家信息</span>
          </Link>
          <Link to={`/business/manage/affiliates/knigh?id=${seller_id}`}>
            <span className={`${style.tabsActive} ${style.marginLeft16}`}>骑士分单规则</span>
          </Link>
        </div>
