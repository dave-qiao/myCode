...
// 编辑提交的函数
  handleSubmit = (e) => {
    ...
    // 获取城市代码
    let city_code = details.d_seller.city_code || details.d_account.city_code;
    const {id} = details.d_seller;

    //修改联系方式规则: 如果输入的是手机号 参数传入shop_mobile 如果输入的是座机号 参数shop_tel
    // 无论传入谁，最后把mobile_tel 参数删掉_mobile_tel
    // 判断手机号的正则
    let mobileRx =/^(0|86|17951)?(13[0-9]|15[012356789]|17[0-8]|18[0-9]|14[57])[0-9]{8}$/;
    // 判断座机号的正则
    let _mobile_tel = this.props.form.getFieldValue('mobile_tel');
        ...
        <Row>
          <Col sm={12}>
            ...
            <FormItem label="商家名称" {...{"labelCol":{"span":4},"wrapperCol":{"span":8}}}>
              <Input {...getFieldProps("name", { initialValue: d_seller.name,
                validate: [
            ...
            <FormItem label="商家类型" {...{"labelCol":{"span":4},"wrapperCol":{"span":8}}}>
              <Select {...getFieldProps("seller_type",{initialValue: d_seller.seller_type + '',
                validate: [
                  { rules: [ { required: true, message: '请选择商家类型'}, ], trigger: 'onBlur', }
                ]
              })}>
                <Option value="1">美食餐饮</Option>
                <Option value="2">生鲜蔬菜</Option>
                <Option value="3">超市商品</Option>
                <Option value="4">鲜花蛋糕</Option>
          ...
          <Col sm={12}>
            ...
            <FormItem label="联系人" {...{"labelCol":{"span":4},"wrapperCol":{"span":8}}}>
              <Input {...getFieldProps("owner_name", { initialValue: d_account.name,
                validate: [
            ...
            <FormItem label="商户状态" {...{"labelCol":{"span":4},"wrapperCol":{"span":8}}}>
              <RadioGroup {...getFieldProps("state",{initialValue:d_seller.state + '' })}>
        ...
        <Row>
          <Col sm={12}>
            <FormItem label="联系方式" {...{"labelCol":{"span":4},"wrapperCol":{"span":8}}}>
              ...
              <Input
                {...{"style":{"width":310}}}
                {...getFieldProps("mobile_tel", { initialValue: _mobile_tel,
                })}
              />
...
let View = ({business_seller,dispatch}) => {
  ...
  const formProps = {
    ...
    details:business_seller.list_details
  }
  return (
...
function mapStateToProps({business_seller,business_public}) {
  return {business_seller,business_public};
};
