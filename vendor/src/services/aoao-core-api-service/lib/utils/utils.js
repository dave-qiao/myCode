...
window.tempAppTool = {
  ...
  numberDateToStr: function numberDateToStr(num) {
    var _date = num + '';
    return _date.substr(0, 4) + '-' + _date.substr(4, 2) + '-' + _date.substr(6, 2);
  },
  sqlit: function sqlit(arr, type) {
    var s = arr[0];
  ...
  utcToDate: function utcToDate(dateStr) {
  ...
  getMonthDays: function getMonthDays(month) {
    var _demo = new Date();
  ...
  dateFormat: function dateFormat(stamp) {
  ...
  err_codeTransform: function err_codeTransform(code) {
    var codeDict = {
      "401001": "参数缺失或错误",
      "401002": "该名称已存在",
      "401003": "该账号已存在",
  ...
  stateTransform: function stateTransform(field, state) {
    var state_dict = {
      sex: {
        '1': '男',
        '2': '女'
...
module.exports = {
  getTOKEN: function getTOKEN() {
    var accountInfo = window.getStorageItem('accountInfo') || '{}';

    var _JSON$parse = JSON.parse(accountInfo),
  ...
  createListParam: function createListParam(params) {
    var _params$page = params.page,
        page = _params$page === undefined ? 1 : _params$page;

    return (0, _extends3.default)({}, params, { page: page, limit: 10 });
  },
  createlimitPage: function createlimitPage(page) {
